...
|`video.txt`|***NEW FILE REQUIREMENT!*** See below for details.|
...
## Video
You will be required to submit a video in which you and your partner present your final project. This video should show off all the features of the site that you have created. We will use this video while grading so be ensure your video is thorough, complete, and follows the instructions below.

You will create a file called `video.txt` which should have a link to the screen recording of your video. Please create the video using Loom. As a student, you get access to a "pro" account for free if you sign up using your `.edu` email. After creating the video, paste the shareable link into the `video.txt` file. Make sure you have given the staff proper access. If we can not access the video through the provided link, you will receive a deduction. The best way to check is to open up the link in an incognito browser and check that you're able to view the video.
...
There is no minimum or maximum length requirement for this video. We encourage you to be concise but thorough so that you are able to fully demonstrate the work you have put in to maximize your grade. This is your chance to show off your project. Anything you want us to see should be included in this video.

This video must be completed after you have finished implementing the project and it is due at the same time the assignment is due. It is very important that you leave an adequate amount of time to complete this portion of the assignment. Submitting the final project without the link to this video will result in a significant deduction.
...
All patterns and practices defined as internal requirements in past assignments continue to apply here (e.g., following code quality guidelines, using the module pattern in front end JavaScript, proper use of `async`/`await` and promises, all errors handled appropriately, `statusCheck` used appropriately in fetch chains, etc.).
...
You must handle errors appropriately throughout the project as outlined in our style guides and reinforced throughout lecture and section.
  ...
  * **IMPORTANT**: Not all server errors that you are expected to handle have been explicitly called out in this specification. Since this is a final project you will need to make choices about when and what errors your API should anticipate in order to have a well designed app. Note that your API should be able to handle errors even if those errors can not be reached through your implementation of the front end of this assignment. You should test your endpoints independently of your front end code (use [not Postman, include link to whatever we decide to use instead]). Try to think of as many edge cases as you can. At this point of the quarter, we expect you to be able to develop thorough error handling. If you have questions, please post on Ed or come to office hours.
...
If we find [inappropriate content or plagiarism](https://docs.google.com/document/d/1Ve5Z3dhcXmqFp-jA3weHqIDDubB-rL0D5d22icL3dYk/preview#heading=h.a8e8oupju3r6) in projects **you will be ineligible for any points on the project**. Ask the instructors if you're unsure if your work is cited appropriately. Any external sources like images should be cited where used in the source code or (ideally) visible in a page footer. Refer to this [copyright example](https://courses.cs.washington.edu/courses/cse154/21au/resources/assets/code-examples/copyright-examples/copyrightexample2.html) page for how to cite images from different sources.
